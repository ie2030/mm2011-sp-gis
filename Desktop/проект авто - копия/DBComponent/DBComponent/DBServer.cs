...
namespace DBComponent
{
    ...
    public class DBServer :IDBServer
    {
        ...
        /// <summary>
        /// Returns nearest points of start node and finish node on road in database
        /// </summary>
        public List<Node> getNearestPoint(Node StartNode, Node FinishNode)
        {
            
            Node start_res = null;
            Node finish_res = null;
            double start_dist = double.MaxValue;
            double finish_dist = double.MaxValue;
            foreach (Node node in nodes)
            {
                if (node.prior == 0) 
                    continue;
                double startDist = countDist(StartNode, node);
                if (startDist < start_dist)
                {
                    start_res = node;
                    start_dist = startDist;
                }
                double finishDist = countDist(FinishNode, node);
                if (finishDist < finish_dist)
                {
                    finish_res = node;
        ...
        public List<Street> getStreets()
        {
            ...
            ///Reding all nodes from database and saving list of them
            nodes = new List<Node>();
            nodes = getNodes();
